...
def open_multidomain_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    total = 0
    for id, dom in enumerate(sorted(os.listdir(source_dir))):
        print(source_dir, dom, id)
        input_images = [str(f) for f in sorted(Path(os.path.join(source_dir, dom)).rglob('*')) if
                        is_image_ext(f) and os.path.isfile(f)]
        total += len(input_images)
        print(total, len(input_images))
    max_idx = maybe_min(total, max_images)
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        if source.rstrip('/').endswith('_lmdb'):
            ...
            return open_multidomain_image_folder(source, max_images=max_images)
