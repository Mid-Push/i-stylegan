...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        if use_label:
            self.label_embed = FullyConnectedLayer(10, embed_features)
            features_list[0] += embed_features
        if c_dim > 0:
            if self.cond_mode in ['flow', 'concat']:
                self.embed = FullyConnectedLayer(c_dim, embed_features)
            if self.cond_mode == 'flow':
                self.flow = DDSF(n_blocks=flow_blocks, hidden_dim=flow_dim)
                self.importance = torch.nn.Parameter(torch.ones([1, i_dim]))
                ...
                layer = FullyConnectedLayer(layer_features, self.flow.num_params*i_dim, lr_multiplier=lr_multiplier)
                setattr(self, f'flow_fc{num_layers}', layer)
                print(self)

            elif self.cond_mode == 'mlp':
                self.importance = torch.nn.Parameter(torch.ones([1, i_dim]))
                flow_features_list = [i_dim] + [layer_features] * (num_layers)
                for mlp_id in range(c_dim):
                    for idx in range(num_layers):
                        in_features = flow_features_list[idx]
                        out_features = flow_features_list[idx + 1]
                        layer = FullyConnectedLayer(in_features, out_features, activation=activation, lr_multiplier=lr_multiplier)
                        setattr(self, f'{mlp_id}mlp_fc{idx}', layer)
                    layer = FullyConnectedLayer(layer_features, i_dim, lr_multiplier=lr_multiplier)
                    setattr(self, f'{mlp_id}mlp_fc{num_layers}', layer)
                print(self)
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False,
        ...
        with torch.autograd.profiler.record_function('input'):
            if self.z_dim > 0:
                ...
                if self.c_dim > 0 and self.cond_mode == 'flow':
                    ...
                    y = layer(y)
                    dsparams = y.view(len(y), self.i_dim, -1)
                    z_same = z[:, self.i_dim:]
                    z_in = z[:, :self.i_dim]
                    z_out = self.flow(z_in, dsparams)
                    z_out = z_in + self.causal_influence() * z_out # parameterization to allow sparsity
                    x = torch.cat([z_out, z_same], 1)
                    if self.flow_norm:
                        style_mask, content_mask = self.get_masks()
                        style_mask = style_mask.expand(x.size(0), style_mask.size(1))
                        content_mask = content_mask.expand(x.size(0), content_mask.size(1))
                        assert x[style_mask].view(x.size(0), -1).size(-1) == torch.sum(self.causal_influence()>0.0)
                ...
                elif self.c_dim > 0 and self.cond_mode == 'mlp':
                    z_same = z[:, self.i_dim:]
                    z_in = z[:, :self.i_dim]
                    outputs = []
                    for mlp_id in range(self.c_dim):
                        zchange = z_in
                        for idx in range(self.num_layers):
                            layer = getattr(self, f'{mlp_id}mlp_fc{idx}')
                            zchange = layer(zchange)
                        layer = getattr(self, f'{mlp_id}mlp_fc{self.num_layers}')
                        zchange = layer(zchange)
                        outputs.append(zchange)
                    out = torch.stack(outputs, dim=1)  # (batch, num_domains, style_dim)
                    idx = torch.LongTensor(range(c.size(0))).to(c.device)
                    z_out = out[idx, c.argmax(dim=1)]  # (batch, sty
                    z_out = z_in + self.causal_influence() * z_out  # parameterization to allow sparsity
                    x = torch.cat([z_out, z_same], 1)
                    if self.flow_norm:
                        style_mask, content_mask = self.get_masks()
                        ...
                        x[style_mask] = normalize_2nd_moment(x[style_mask].view(x.size(0), -1)).view(-1)
                        x[content_mask] = normalize_2nd_moment(x[content_mask].view(x.size(0), -1)).view(-1)
            if self.c_dim > 0 and self.cond_mode == 'concat':
                misc.assert_shape(c, [None, self.c_dim])
                y = normalize_2nd_moment(self.embed(c.to(torch.float32)))
                x = torch.cat([x, y], dim=1) if x is not None else y

        if self.use_label and label is not None:
            x = torch.cat([x, normalize_2nd_moment(self.label_embed(label.float()))], 1)
        # Main layers.
        for idx in range(self.num_layers):
        ...
        if sparse_loss and self.cond_mode in ['flow','mlp']:
            return x, torch.mean(torch.abs(self.importance))
        else:
            return x
